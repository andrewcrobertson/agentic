...
- Add root `package.json` with pnpm workspaces configuration and `@agentic/` package scope convention
...
- New packages: `packages.lib/system.logging` (`@agentic/system.logging`), `packages.apps/api` (`@agentic/api`)
- New CI/CD workflows:
  - `.github/workflows/ci.yml` — lint, typecheck, test, build (with turbo cache)
  - `.github/workflows/publish.yml` — driven by changesets (`@changesets/action`): opens "Version Packages" PR on changes to main, then publishes `@agentic/api` to GitHub Packages and pushes Docker image to ghcr.io on merge
